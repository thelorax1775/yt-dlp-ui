    ...
    <div className="flex flex-col gap-4 rounded-xl border bg-card p-4">
      <div className="grid gap-2">
        ...
        <Select value={mode} onValueChange={(v) => setMode(v as Mode)}>
          ...
          <SelectContent>
            <SelectItem value="bestvideo">Best video (video + audio)</SelectItem>
            <SelectItem value="bestaudio">Best audio (extract)</SelectItem>
        ...
        <div className="grid gap-2">
          ...
          {metadata.formats.length > 0 && (
            <p className="text-xs text-muted-foreground">
              {metadata.formats.length} formats available — use a format_id from
              yt-dlp.
            </p>
          )}
        </div>
